          ...
          if (selection && selection.rangeCount > 0 && selection.toString().length > 0) {
            ...
            try {
              ...
              if (selectedText && selectedText.trim()) {
                ...
                const updateFontSizeInSelection = (node) => {
                  if (node.nodeType === Node.TEXT_NODE) {
                    ...
                    if (tagName === 'span') {
                      ...
                      const newSpan = node.cloneNode(false); // Clone without children
                      newSpan.style.fontSize = pixelSize + "px";
                      
                      // Process children and add them to the updated span
                      for (let i = 0; i < node.childNodes.length; i++) {
                        const updatedChild = updateFontSizeInSelection(node.childNodes[i]);
