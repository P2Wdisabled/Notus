...
export default function EditDocumentPageClient(props: EditDocumentPageClientProps) {
  ...
  const realtimeUserId = toPositiveNumber(userId) ?? toPositiveNumber((props.session as any)?.user?.id);
  ...
  const handleSubmit = useCallback(
    async (e?: React.FormEvent) => {
      ...
      let drawingsToSave: any[] = Array.isArray(content.drawings) ? content.drawings : [];

      if (canvasCtrl && typeof canvasCtrl.saveDrawings === "function") {
        try {
          drawingsToSave = await canvasCtrl.saveDrawings({ force: true });
        } catch {
          drawingsToSave = Array.isArray(content.drawings) ? content.drawings : [];
        }
      }
  ...
  }, [document, content.text, handleSubmit, normalizeContent, updateLocalStorage, buildContentSnapshot, createPersonalCopyFromOffline, userId, realtimeUserId, title, tags]);
