...
import { Button, Card, Form, Input, Modal } from "@/components/ui";
...
export default function EditProfilePageClient({ user }) {
  ...
  const displayName = initial.name || `${initial.firstName} ${initial.lastName}`.trim() || initial.username || "MonCompte";
  ...
  useEffect(() => {
    ...
    const doUpdate = async () => {
      ...
      try {
        ...
        console.error("Erreur lors de l'enregistrement de la session locale:", e);
      }
  ...
  }, [message, firstName, lastName, username, email, user?.id]);
  ...
  return (
    <div>
      ...
      <div className="mt-6">
        <h2 className="font-title text-3xl md:text-4xl text-black dark:text-white mb-4">Informations personnelles</h2>
        ...
        <Card className="px-3 py-6">
          <form action={handleSubmit} className="space-y-4">
            ...
            {message && (
              <p className={`text-sm ${message.toLowerCase().includes("succès") ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}>
                {message}
              </p>
            )}

            <div className="flex justify-center pt-2 gap-4">
              <Button type="submit" loading={isPending} variant="primary" className="px-6 py-2">
                Mettre à jour
              </Button>
              <Button type="button" variant="secondary" className="px-6 py-2" onClick={() => router.push("/profile")}>
                Annuler
              </Button>
            </div>
          </form>
