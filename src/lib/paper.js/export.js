...
// Export to text file
export const exportToPDF = async (content, filename = 'document') => {
  try {
    console.log('Starting PDF export with content:', content.substring(0, 100) + '...');
    
    // Get the actual contentEditable element from the page
    const originalEditor = document.querySelector('[contenteditable="true"]');
    ...
    const clonedContent = originalEditor.cloneNode(true);
...
export const exportToWord = async (content, filename = 'document') => {
  try {
          ...
          if (text.trim()) {
            ...
            if (highlightColor) {
              ...
              if (predefinedHighlights[highlightColor]) {
                textRunWithBackground.highlight = predefinedHighlights[highlightColor];
              ...
              console.log(`🔧 FINAL CONFIG:`, {
                ...
                highlight: textRunWithBackground.highlight,
                shading: textRunWithBackground.shading
              });
...
// Export to TXT
export const exportToTXT = (content, filename = 'document') => {
  try {
...
export const exportDocument = async (options) => {
