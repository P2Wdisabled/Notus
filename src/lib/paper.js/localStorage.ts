// lib/localStorage.ts
import { LocalStorageData, TextFormatting, Settings, SerializedPathData } from './types';
...
export const saveDrawingsToLocalStorage = (drawings: SerializedPathData[]): boolean => {
  if (!isLocalStorageAvailable()) {
...
export const saveTextToLocalStorage = (text: string): boolean => {
  if (!isLocalStorageAvailable()) {
    // console.warn('localStorage is not available');
    return false;
  }
...
export const saveTextFormattingToLocalStorage = (formatting: TextFormatting): boolean => {
  if (!isLocalStorageAvailable()) {
